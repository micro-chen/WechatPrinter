...
namespace WechatPrinter.Support
{
    class WechatPrinterConf
    {
        ...
        private const double PRINT_WIDTH = 832 - 832 * 0.0377 ;
        //private const double PRINT_HEIGHT = 1107;
        private const double PRINT_HEIGHT = PRINT_WIDTH * 1.33;
        private const double PRINT_HEIGHT_POS = 832 * 0.0377 * (SCREEN_DPI / PRINT_DPI);
        private const double PRINT_WIDTH_POS = 832 * 0.0377 * (SCREEN_DPI / PRINT_DPI);
        
        private const double PRINT_LOGO_HEIGHT = 27;
        private const double PRINT_LOGO_HEIGHT_POS = PRINT_HEIGHT * (SCREEN_DPI / PRINT_DPI) + 4 + PRINT_HEIGHT_POS;
        private const double PRINT_LOGO_WIDTH_POS = PRINT_WIDTH_POS;
        //private const double PRINT_LOGO_WIDTH_POS = PRINT_WIDTH * (SCREEN_DPI / PRINT_DPI) / 2 - (PRINT_LOGO_HEIGHT * (SCREEN_DPI / PRINT_DPI) * 5.65) / 2 + PRINT_WIDTH_POS;
        private const double PRINT_DPI = 300;
        private const double SCREEN_DPI = 96;

        private const int PRINT_IMG_TIMER_INTERVAL = 5 * 1000;

        private const int PRINT_WAIT_TIME = 0 * 1000;
